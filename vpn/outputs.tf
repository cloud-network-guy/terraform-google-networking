  ...
  value = [for i, v in local.vpn_tunnels :
    {
      ...
      cloud_vpn_gateway_ip    = try(data.google_compute_ha_vpn_gateway.default.vpn_interfaces.*.ip_address[v.tunnel_index], "unknown")
      ...
      cloud_router_bgp_asn    = try(data.google_compute_router.default[local.router].bgp[0].asn, "ERROR")
    }
  ]
}
