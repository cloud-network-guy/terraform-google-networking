...
| <a name="input_dns_zones"></a> [dns\_zones](#input\_dns\_zones) | List of Private DNS Zones | <pre>map(object({<br/>    create      = optional(bool, true)<br/>    dns_name    = string<br/>    name        = optional(string)<br/>    description = optional(string)<br/>    records = optional(list(object({<br/>      create  = optional(bool, true)<br/>      key     = optional(string)<br/>      name    = string<br/>      type    = optional(string)<br/>      ttl     = optional(number)<br/>      rrdatas = list(string)<br/>    })))<br/>  }))</pre> | `{}` | no |
